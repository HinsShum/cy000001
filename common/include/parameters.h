...
/*---------- type define ----------*/
struct st_para_info {
    uint8_t *start_address;     /*<< the start address of the flash, 4-byte aligned */
    uint8_t *end_address;       /*<< the end address of the flash, 4-byte aligned */
    uint32_t para_size;         /*<< para size, n * para size = block size */
    uint32_t erase_block_size;  /*<< the min size of the erase block size */
    void *para_point;           /*<< the lastest parameter table point */
 ...
 * @param new_para: is the address of the parameter block to be
 * written to flash
 *
 ...
 * @retval None
 */
extern void *parameters_get(struct st_para_info *pinfo);
